      ...
      <div className="authForm">
        ...
        {!token ? (
          ...
          <form className="formGrid" onSubmit={submit}>
            <input aria-label="New password" placeholder="New password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} />
            <input aria-label="Confirm new password" placeholder="Confirm new password" type="password" required value={confirm} onChange={(e) => setConfirm(e.target.value)} />
            {error && <p style={{ color: '#c00', fontSize: 13, margin: 0 }}>{error}</p>}
            <button className="blackButton" disabled={busy || !password || !confirm}>{busy ? 'SAVING…' : 'SET NEW PASSWORD'}</button>
          </form>
        )}
